...
def test_dead_pid_marks_stalled(tmp_path):
    _write_progress(
        ...
        enriched_count=3,
        in_progress=True,
        background_pid=999999,
        session_fingerprint=_FP,
        bonds_covered=False,
    )
    status = get_enrichment_status(tmp_path)
...
_DISPLAY_PATTERN = re.compile(r"^[✅⏳⚠️].+\d+/\d+\s*·\s*[\d.]+%\s*·\s*[0-9a-f]{8}")


@pytest.mark.parametrize(
    "enriched,total,in_progress,pid,expected_emoji",
    [
...
def test_display_format_matches_pattern(
    tmp_path, enriched, total, in_progress, pid, expected_emoji
):
    _write_progress(
        tmp_path,
        total_symbols=total,
...
def test_chain_is_order_dependent():
    ...
    fp_msft_first = update_session_fingerprint(
        update_session_fingerprint(fp0, "MSFT", "synthesis A"), "AAPL", "synthesis B"
    )
    fp_aapl_first = update_session_fingerprint(
        update_session_fingerprint(fp0, "AAPL", "synthesis B"), "MSFT", "synthesis A"
    )
