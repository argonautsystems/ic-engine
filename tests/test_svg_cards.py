...
SYMBOL = "MSFT"
SYNTHESIS = (
    "Azure cloud growth accelerating with 31% YoY revenue increase driven by AI services adoption."
)
...
def test_path_traversal_in_symbol_sanitized(tmp_path):
    out_path = render_card(
        "../../../etc/passwd", SYNTHESIS, ATTRIBUTION, FINGERPRINT, TIMESTAMP, tmp_path
    )
...
def test_synthesis_wraps_long_text(tmp_path):
    ...
    synthesis_lines = re.findall(
        r'<text x="24" y="\d+" font-family="monospace" font-size="13"', svg
    )
