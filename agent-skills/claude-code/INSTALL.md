...
## Install from the InvestorClaw repo
...
Claude Code and Claude Desktop install this repo's container-first plugin:
the same skill bundle, same Docker Compose runtime, and same MCP-HTTP
endpoint as every other agent. The plugin writes its MCP server entries
into `claude_desktop_config.json` and registers `/ask` + `/refresh` as
slash commands. Restart Claude Code so it picks up the new MCP servers.
...
`localhost:5002` (memory) and `ghcr.io/argonautsystems/ic-engine:4.6.0-cpu` on
