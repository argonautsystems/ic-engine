...
def _safe_url(url: Any) -> str:
    """Return URL if it uses an allowed scheme (http/https), else empty string."""
    if not url:
        return ""
    s = str(url).strip()
    lowered = s.lower()
...
<div class="hdr" style="background:{_C_HEADER_BG};padding:28px 32px;border-radius:8px 8px 0 0;">
  ...
  <div style="color:{_C_NEUTRAL};font-size:13px;">
    {_esc(date)} &nbsp;|&nbsp; Portfolio Value: {_currency(total_value)}
  </div>
...
<tr>
  ...
  <td style="padding:4px 0 4px 8px;font-size:12px;font-weight:600;color:{_C_NEGATIVE};">
    ...
    alloc_table = (
        f'<table style="border-collapse:collapse;margin-top:12px;">{"".join(alloc_rows)}</table>'
    )
...
def _render_top_holdings(holdings: dict, analyst_data: Optional[dict]) -> str:
  ...
  {_td(_esc(sector), f"color:{_C_LABEL};font-size:12px;")}
  {_td(_currency(value))}
  {_td(f"{weight:.1f}%")}
  {_td(f'<span style="color:{_sign_color(gl_pct)};font-weight:600;">{_signed(gl_pct)}</span>')}
  {_td(f'<span style="color:{consensus_color};font-weight:600;">{_esc(consensus)}</span>{upside_str}')}
</tr>""")
...
def _render_analyst_summary(analyst: dict) -> str:
    ...
    kpis = [
        ...
        (
            "Strong Coverage",
            f"{int(cov.get('strong_coverage', 0))} ({strong_pct:.0f}%)",
            _C_POSITIVE,
        ),
        (
            "Moderate Coverage",
            f"{int(cov.get('moderate_coverage', 0))} ({mod_pct:.0f}%)",
            _C_ACCENT,
        ),
...
def _render_news_summary(news: dict) -> str:
    ...
    posture_badge = (
        ...
        f'font-size:12px;">{_esc(posture.upper())}</span>'
    )

    narrative_html = (
        f'<p style="font-size:13px;color:{_C_TEXT_BODY};margin:10px 0;line-height:1.6;">{_esc(narrative)}</p>'
        if narrative
        else ""
    )
    ...
    def _news_item_card(item: dict, color: str) -> str:
        ...
        url = _safe_url(item.get("url") or item.get("link") or "")
        safe_title = _esc(title)
        title_html = (
            f'<a href="{url}" target="_blank" rel="noopener noreferrer" '
            f'style="color:{_C_ACCENT};text-decoration:underline;">{safe_title}</a>'
            if url
            else f'<span style="color:{_C_TEXT_BODY};">{safe_title}</span>'
        )
        ...
        return (
            ...
            f'<span style="color:{_C_LABEL};">[{_esc(item.get("symbol", "?"))}]</span> '
            f"{title_html}</div>"
            f"{impact_html}"
            f"</div>"
        )
    ...
    def _linked_li(title: str, color: str) -> str:
        url = _safe_url(url_map.get(title) or url_map.get(title[:80]) or "")
        sym = sym_map.get(title) or sym_map.get(title[:80]) or ""
        sym_tag = (
            f'<span style="font-size:10px;font-weight:700;color:{_C_LABEL};">[{_esc(sym)}]</span> '
            if sym
            else ""
        )
        safe_title = _esc(title)
...
def _render_bond_summary(bonds: Optional[dict]) -> str:
    ...
    kpis = [
        ...
        (
            "Duration Risk",
            _esc(dur_risk),
            _C_HIGH if str(dur_risk).lower() == "high" else _C_NEUTRAL,
        ),
    ]
    kpi_row = _kpi_row(kpis)
    ...
    if avg_credit:
        extras.append(
            ...
            f'<strong style="color:{_C_TEXT_PRIMARY};">{_esc(avg_credit)}</strong></span>'
        )
    if tax_savings >= 500:
        extras.append(
...
<tr>
  <td style="padding:3px 8px 3px 0;font-size:12px;color:{_C_LABEL};white-space:nowrap;width:60px;">{_esc(bucket)}</td>
...
def _render_performance_summary(performance: dict) -> str:
    ...
    kpis = [
        ("Period Start", _esc(period), _C_ACCENT),
...
<div style="margin-bottom:12px;padding:12px 14px;border-radius:6px;
  ...
  <div style="margin-bottom:5px;">
    ...
    <span style="font-size:10px;font-weight:700;color:{color};text-transform:uppercase;
                 letter-spacing:0.06em;margin-right:6px;">[{_esc(label)}] {_esc(cat)}</span>
    <span style="font-size:13px;font-weight:600;color:{_C_TEXT_PRIMARY};">{_esc(title)}</span>
  </div>
  <div style="font-size:12px;color:{_C_TEXT_BODY};line-height:1.6;">{_esc(detail)}</div>
</div>""")

    return _section(f"FA Discussion Topics ({len(fa_topics)} Items)", _card("".join(items_html)))
...
<head>
  ...
  <title>InvestorClaw EOD Report \u2014 {_esc(date)}</title>
  <style type="text/css">
{_MOBILE_CSS}
  </style>
</head>
