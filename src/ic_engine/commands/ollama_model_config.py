...
"""
Local Inference Model Configuration for InvestorClaw GPU Inference.

Defines the gemma4-consult model configuration and provides helpers to
replicate it on any compatible llama-server or Ollama endpoint.

RECOMMENDED: llama-server (llama.cpp) — 131K context, less VRAM overhead
LEGACY: Ollama — 4K context default, larger daemon overhead

gemma4-consult is a tuned derivative of gemma4:e4b optimised for low-latency
consultative Q&A. It is the recommended INVESTORCLAW_CONSULTATION_MODEL.

Benchmarked on: local-inference — RTX 4500 Ada 24 GB, driver 595.58.03, llama-server
  gemma4:e4b (llama-server):  ~73 tok/s, 131K context, simple Q&A 1.5-2.5s, complex 8-14s
  gemma4:e4b (Ollama):        ~66 tok/s, 4K context,   simple Q&A 2.0-3.0s, complex 12-18s

Hardware requirements for gemma4:e4b:
  - CUDA compute capability >= 8.0 (RTX 30xx / A-series or newer)
  - >= 12 GB VRAM (model is ~9.6 GB Q6_K)
  - llama-server >= b3319 (preferred) or Ollama >= 0.20.x with Flash Attention support
...
if __name__ == "__main__":
    ...
    # Check if Ollama endpoint is reachable before attempting setup
    try:
        requests.get(f"{args.endpoint}/api/tags", timeout=2)
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        print(
            json.dumps(
                {
                    "status": "skipped",
                    "reason": f"Ollama not available at {args.endpoint}",
                    "hint": "Start Ollama or set INVESTORCLAW_CONSULTATION_ENDPOINT",
                    "ic_result": {"script": "ollama_model_config.py", "exit_code": 0},
                }
            )
        )
        sys.exit(0)
