    ...
    for pos in top_equity:
        ...
        if w >= 10.0:
            espp_note = (
                ...
                if is_espp
                else " Concentrated positions amplify both upside and downside. "
            ...
            topics.append(
                {
                    ...
                    + (
                        " · Managed: " + f"{_managed_value / total_value:.0%}"
                        if _managed_value > 0
                        else ""
                    ),
                }
        ...
        topics.append(
            {
                ...
                "detail": (
                    ...
                    + (
                        ...
                        if unrealized_gl < 0
                        else "Significant unrealized gains may create a tax liability upon sale. "
...
def _topics_from_news(news_data: dict) -> List[dict]:
    ...
    macro_themes: List[str] = [t["theme"] if isinstance(t, dict) else t for t in _raw_themes]
...
def _topics_from_bonds(bond_data: dict) -> List[dict]:
    ...
    ps.get("recommendations", []) or []
    ps.get("average_credit_quality", "") or ""
        ...
        topics.append(
            {
                ...
                "detail": (
                    ...
                    + (
                        ...
                        if w_sharpe < 0
                        else "A Sharpe below 0.5 suggests the portfolio's returns have not well-compensated for the volatility taken."
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Extract FA discussion topics from portfolio reports"
    )
    parser.add_argument(
        "--reports-dir", help="Path to reports directory (default: today's dated dir)"
    )
