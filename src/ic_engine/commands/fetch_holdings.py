...
from ic_engine.models.cdm_portfolio import (
...
def _is_espp_holding(
    symbol: str, account_name: Optional[str], espp_holdings: Dict[str, List[str]]
) -> bool:
    """
    ...
    return any(account_clean == acc or acc in account_clean for acc in espp_accounts)
...
def _fetch_security_info_batch(
    symbols: List[str], max_workers: int = 8, timeout: int = 30
...
def _build_compact_holdings(
    ...
    try:
        ...
        sector_warning_pct = thresholds.get("sector_warning", 0.25) * 100
    except Exception:
        sector_warning_pct = 25.0
    if sector_weights and any(w > sector_warning_pct for w in sector_weights.values()):
        warning_msg = (
            "⚠️  CONCENTRATION ALERT: One or more sectors exceed 25% of portfolio.\n"
            "   Review diversification and discuss with your advisor."
        )
    ...
    espp_sector_pct = (
        ...
        if equity_value and espp_value_by_sector
        else {}
    )
    ...
    # Derive dominant price provider from equity holdings for DEG-C2 contract
    _provider_counts: dict = {}
    for _h in equity_data.values():
        _dp = getattr(_h, "data_provider", None) or "unknown"
        _provider_counts[_dp] = _provider_counts.get(_dp, 0) + 1
    _provider_used = (
        max(_provider_counts, key=_provider_counts.get) if _provider_counts else "yfinance"
    )

    # CDM 5 extension: top-N lists for crypto / futures / metals (same pattern as top_equity)
    def _top_n(asset_data: dict, n: int = 10) -> list:
        rows = sorted(asset_data.values(), key=lambda h: h.value, reverse=True)[:n]
        out = []
        for _h in rows:
            _entry = {
                "symbol": _h.symbol,
                "value": round(_h.value, 2),
                "weight_pct": round(_h.value / total_value * 100, 2) if total_value else 0,
                "gl_pct": round(_h.unrealized_gain_loss_pct * 100, 2),
                "type": _h.asset_type,
            }
            # Surface CDM 5 extension fields when present
            if getattr(_h, "blockchain", None):
                _entry["blockchain"] = _h.blockchain
            if getattr(_h, "contract_symbol", None):
                _entry["contract_symbol"] = _h.contract_symbol
            if getattr(_h, "expiry_date", None):
                _entry["expiry_date"] = _h.expiry_date
            if getattr(_h, "notional_value", None) is not None:
                _entry["notional_value"] = round(_h.notional_value, 2)
            if getattr(_h, "metal_type", None):
    ...
    compact = {
        ...
        "summary": {
            ...
            # CDM 5 extension totals — always present (0.0 when absent) for schema stability
            "crypto_value": round(crypto_value, 2),
            "futures_value": round(futures_value, 2),
            "metals_value": round(metals_value, 2),
            ...
            "crypto_pct": round(crypto_value / total_value * 100, 1) if total_value else 0,
            "futures_pct": round(futures_value / total_value * 100, 1) if total_value else 0,
            "metals_pct": round(metals_value / total_value * 100, 1) if total_value else 0,
            "position_count": {
    ...
    # Only emit CDM 5 extension top-N lists when the bucket is non-empty,
    # to keep compact output lean for equity-only portfolios.
    if top_crypto:
        compact["top_crypto"] = top_crypto
    if top_futures:
    ...
    if _acct_totals:
        compact["accounts"] = {
            ...
            for name, data in sorted(
                _acct_totals.items(), key=lambda x: x[1]["value"], reverse=True
            )
        }
...
class PortfolioFetcher:
    def __init__(self):
        ...
        self.cash_data = {}
        self.margin_data = {}
        # CDM 5 extension buckets — empty unless detected by normalize step 7
    ...
    def _normalize_broker_columns(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        rename_map = {
            k: v for k, v in col_aliases.items() if k in df.columns and v not in df.columns
        }
        if rename_map:
        ...
        _has_nulls = _has_asset_type_col and df.select(pl.col("asset_type").is_null().any()).item()
        if not _has_asset_type_col or _has_nulls:
            ...
            cash_by_null = pl.col("symbol").is_null() & (
            ...
            )  # default to muni if no description

            # ---- CDM 5 extension: crypto / futures / metals detection ----
            # Crypto: symbol ends with '-USD' (yfinance convention: BTC-USD, ETH-USD) OR
            #         symbol is in a known cryptocurrency ticker set.
            _crypto_syms = [
                "BTC",
                "ETH",
                "SOL",
            ...
            crypto_by_symbol = (
                (
                    ...
                    & (
                        pl.col("symbol").str.to_uppercase().str.ends_with("-USD")
                        | pl.col("symbol").str.to_uppercase().is_in(_crypto_syms)
                    )
                )
                if has_symbol
                else pl.lit(False)
            ...
            bond_count = df.filter(pl.col("asset_type").is_in(["bond", "municipal_bond"])).height
            logger.info(
                f"Inferred asset_type: {bond_count} bond/muni rows detected via CUSIP heuristic"
            )
    ...
    def _read_xls_to_polars(input_file: str) -> pl.DataFrame:
        ...
        raw_headers = [str(sheet.cell_value(data_start, c)).strip() for c in range(sheet.ncols)]
    ...
    def load_portfolio_file(self, input_file: str) -> pl.DataFrame:
        ...
        try:
            ...
            if ext == ".xls":
                ...
                try:
                    peek = pl.read_excel(
                        input_file, read_options={"n_rows": 1, "has_header": False}
                    )
                ...
                # Strip UTF-8 BOM (EF BB BF) from first line so Windows-exported
                # CSVs (Excel-saved) don't break header comparison.
                with open(input_file, "rb") as _f:
                    raw_first = _f.readline()
                if raw_first.startswith(b"\xef\xbb\xbf"):
                    raw_first = raw_first[3:]
                first_line = raw_first.decode("utf-8", errors="replace").strip().strip('"')
                ...
                df = pl.read_csv(
                    ...
                    # utf8-lossy tolerates BOM + mixed encodings from Windows exports
                    encoding="utf8-lossy",
                )
    ...
    def validate_holdings_data(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if not (has_symbol and has_quantity and has_type):
            missing = []
            if not has_symbol:
                missing.append("symbol or cusip")
            if not has_quantity:
                missing.append("shares or market_value")
            if not has_type:
        ...
        if "purchase_price" not in df.columns:
            if "price" in df.columns:
                ...
                logger.warning(
                    "Using current 'price' as purchase_price (historical data unavailable)"
                )
            else:
    ...
    def fetch_equity_holdings(self, equity_df: pl.DataFrame) -> Dict:
        ...
        # Load account display-name overrides (e.g. (BX 12345) → Retirement 401K)
        ...
        for _row in equity_df.to_dicts():
            ...
            if isinstance(_acct, str):
                _acct = _acct.strip()
            # Apply account label override (e.g. "(BX 12345)" → "Retirement 401K")
            ...
            if _agg[_agg_key]["_espp_account"] is None and _is_espp_holding(
                _sym, _acct, espp_holdings
            ):
            ...
            if mv and mv > 0 and price and price > 0 and not _refresh_prices:
                ...
                if proxy:
                    ...
                    if _pp and _pp > 0:
                        ...
                        broker_only[agg_key] = {
                            "price": _pp,
                            "market_value": _pp * _sh_val,
                            "sym": sym,
                        }
                        logger.info(
                            f"{sym}: using purchase_price=${_pp:.4f} as NAV (mutual fund, no proxy)"
                        )
                    else:
                        needs_live.append(sym)
            else:
        ...
        if broker_only and not _refresh_prices:
            logger.info(
                f"Using broker-supplied prices for {len(broker_only)} symbols (refresh disabled)"
            )
        ...
        if _missing:
            logger.info(
                f"Fetching security info for {len(_missing)} symbols (cached: {len(_effective_syms) - len(_missing)})"
            )
            try:
        ...
        for agg_key in symbols:
            try:
                ...
                try:
                    cost_basis_total = (
                        float(cost_basis_total) if cost_basis_total is not None else None
                    )
                except (ValueError, TypeError):
                ...
                if _row_asset_type == "mutual_fund":
                    ...
                    _sector = row.get("sector") or (
                        _sec_info.get("sector", "Unknown")
                        if isinstance(_sec_info, dict)
                        else "Unknown"
                    )
                else:
                    _security_type = (
    ...
    def fetch_bond_holdings(self, bond_df: pl.DataFrame) -> Dict:
        ...
        for row in bond_df.to_dicts():
            try:
                ...
                row.get("cusip", row.get("symbol", "UNKNOWN"))
                ...
                try:
                    market_value_raw = (
                        float(market_value_raw) if market_value_raw is not None else None
                    )
                except (ValueError, TypeError):
                ...
                if market_value_raw is not None and market_value_raw > 0:
                    ...
                    current_price = (
                        ...
                        else (value / quantity if quantity else 100.0)
                    )
                else:
                ...
                if maturity_raw in ("", "N/A", "None", "nan") and bond_name:
                    m = re.search(
                        r"MATURES?\s+(\d{1,2}/\d{1,2}/\d{2,4})", str(bond_name), re.IGNORECASE
                    )
                ...
                logger.error(
                    f"Error processing bond {row.get('cusip', row.get('symbol', 'unknown'))}: {e}"
                )
    ...
    def fetch_cash_holdings(self, cash_df: pl.DataFrame) -> Dict:
        ...
        for idx, row in enumerate(cash_df.to_dicts()):
            try:
                ...
                try:
                    interest_rate = (
                        float(interest_rate_raw) / 100 if interest_rate_raw is not None else 0.0
                    )
                except (ValueError, TypeError):
                    interest_rate = 0.0
    ...
    def fetch_margin_holdings(self, margin_df: pl.DataFrame) -> Dict:
        ...
        for row in margin_df.to_dicts():
            try:
                ...
                logger.info(
                    f"Margin {loan_id}: Principal ${principal:,.2f} @ {interest_rate * 100:.2f}% = Total ${total_debt:,.2f}"
                )
    ...
    def _convert_to_cdm_portfolio(
        ...
        # ---- CDM 5 extension: crypto positions ----
        # CDM 5 payout mapping: CryptoPayout with cryptoAsset.identifier.
        # We serialize using the established Position+Asset shape but set
        # securityType="Crypto" so normalize_portfolio() routes to the crypto bucket.
        for _key, holding in crypto_data.items():
            pos = Position(
                product=Product(
                    product_identifier=ProductIdentifier(
                        identifier_type="CRYPTO_SYMBOL",
                        identifier=holding.symbol,
                    )
                ),
                asset=Asset(
                    ...
                    security_name=holding.symbol,
                    sector=(holding.blockchain or "Cryptocurrency"),
                ),
                price_quantity=PriceQuantity(
                    quantity=Quantity(amount=holding.shares, unit="units"),
                    current_price=Price(amount=holding.current_price, currency="USD"),
        ...
        # ---- CDM 5 extension: futures positions ----
        # CDM 5 payout mapping: InterestRatePayout (financial futures) or
        # CommodityPayout (metals/energy futures). We emit securityType="Futures"
        # uniformly; downstream analytics can split by contract_symbol / expiry_date.
        for _key, holding in futures_data.items():
            pos = Position(
                product=Product(
                    product_identifier=ProductIdentifier(
                        identifier_type="CONTRACT_SYMBOL",
                        identifier=(holding.contract_symbol or holding.symbol),
                    )
                ),
                asset=Asset(
                    product_identifier=ProductIdentifier(
        ...
        # ---- CDM 5 extension: metals positions ----
        # CDM 5 payout mapping: CommodityPayout with commodity.commodityType=PRECIOUS_METALS.
        # ETF/spot-proxy positions route through equity already; this bucket covers
        # physical / dedicated metals holdings.
        for _key, holding in metals_data.items():
            pos = Position(
                product=Product(
                    product_identifier=ProductIdentifier(
                        identifier_type="TICKER",
                ...
                asset=Asset(
                    ...
                    security_type="Commodity",
                    asset_class="PreciousMetals",
                    security_name=(holding.name or holding.symbol),
                    sector=(holding.metal_type or "Precious Metals"),
                ),
                price_quantity=PriceQuantity(
                    quantity=Quantity(
                        amount=holding.shares,
                        unit=("troy_oz" if holding.troy_oz else "units"),
                    ),
                    current_price=Price(amount=holding.current_price, currency="USD"),
                    cost_basis_price=Price(amount=holding.purchase_price, currency="USD"),
                ),
    ...
    def main(self, input_file: str, output_file: str = "holdings.json") -> None:
        ...
        try:
            ...
            equity_df = (
                df.filter(pl.col("asset_type").str.to_lowercase().is_in(["equity", "mutual_fund"]))
            ...
            if "asset_type" in df.columns:
                ...
                bond_df = df.filter((asset_lower == "bond") | (asset_lower == "municipal_bond"))
            else:
                bond_df = pl.DataFrame()

            cash_df = (
                df.filter(pl.col("asset_type").str.to_lowercase() == "cash")
                if "asset_type" in df.columns
                else pl.DataFrame()
            )
            margin_df = (
                df.filter(pl.col("asset_type").str.to_lowercase() == "margin")
                if "asset_type" in df.columns
                else pl.DataFrame()
            )

            # CDM 5 extension: crypto / futures / metals.
            # Route each through fetch_equity_holdings() (the safer approach: reuses
            # yfinance fetch + Holding construction) but preserve asset_type on the
            # resulting Holding objects so downstream predicates (is_crypto/...) work.
            if "asset_type" in df.columns:
                _al = pl.col("asset_type").str.to_lowercase()
            ...
            # CDM 5 extension fetches — reuse the equity pipeline but overwrite
            # asset_type so the Holding predicates classify correctly.
            self.crypto_data: dict = {}
            self.futures_data: dict = {}
            self.metals_data: dict = {}
            if len(crypto_df) > 0:
            ...
            if len(futures_df) > 0:
                _futures = self.fetch_equity_holdings(futures_df)
                for _k, _h in _futures.items():
                    _h.asset_type = "futures"
            ...
            for _acct_data in _accounts.values():
                _total = (
                    _acct_data["etf_count"] + _acct_data["fund_count"] + _acct_data["equity_count"]
                )
            ...
            sum(h.unrealized_gain_loss for h in self.equity_data.values()) + sum(
                h.unrealized_gain_loss for h in self.bond_data.values()
            )
            ...
            if is_interactive():
                # Render formatted table for interactive terminals (Claude Code, bash)
                summary = compact.get("summary", {})
                print(format_header("Portfolio Summary"))
                print(
                    f"  {Colors.CYAN}Total Value{Colors.RESET}: {format_currency(summary.get('total_value', 0))}"
                )
                print(
                    f"  {Colors.CYAN}Equities{Colors.RESET}: {format_currency(summary.get('equity_value', 0))} ({summary.get('equity_pct', 0):.1f}%)"
                )
                print(
                    f"  {Colors.CYAN}Bonds{Colors.RESET}: {format_currency(summary.get('bond_value', 0))} ({summary.get('bond_pct', 0):.1f}%)"
                )
                print(
                ...
                if compact.get("top_equity"):
                    print(format_header("Top 5 Holdings", width=50))
            ...
            secure_file_permissions(summary_path)
...
if __name__ == "__main__":
    # Extract --artifact PATH and --stonkmode (pass-through from router).
    # Removed from sys.argv so the legacy positional parsing below still works.
    ...
    # Router auto-appends --verbose; fetch_holdings parses argv positionally
    # (input_file = argv[1], output_file = argv[2]) so --verbose would land
    # as output_file. Strip it here; fetch_holdings has no verbose mode.
    pop_argv_flag(_argv, "--verbose")
    sys.argv = _argv

    if len(sys.argv) < 2:
        print("Usage: fetch_holdings.py <input_file> [output_file] [--artifact PATH] [--stonkmode]")
    ...
    # Optional HTML artifact — read the holdings_summary.json we just wrote.
    if _artifact_path:
        try:
            _summary_path = Path(output_file).expanduser().parent
            if _summary_path.name == ".raw":
                _summary_path = _summary_path.parent
            _summary_path = _summary_path / "holdings_summary.json"
            with open(_summary_path) as _sf:
                _compact = json.load(_sf)
            _out = build_holdings_artifact(_compact, _artifact_path, stonkmode=_stonkmode)
