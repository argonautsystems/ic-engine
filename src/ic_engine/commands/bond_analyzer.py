...
# resolve whether invoked as a module or a standalone script.
...
class BondAnalyzer:
    ...
    def __init__(self):
        ...
        self._fred_degraded: bool = False
    ...
    def _load_treasury_yields(self):
        ...
        if not api_key:
            ...
            self._fred_degraded = True
            return

        fetched: Dict[str, float] = {}
        with ThreadPoolExecutor(max_workers=len(self.TREASURY_ENDPOINTS)) as executor:
            futures = {
                executor.submit(self._fred_fetch_series, series_id, api_key): tenor
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.debug(f"Treasury yield fetch failed for {tenor}: {e}")

        if fetched:
            self.treasury_yields = fetched
            logger.info(
                f"Loaded {len(fetched)}/{len(self.TREASURY_ENDPOINTS)} Treasury yields from FRED"
            )
        else:
    ...
    def _load_tips_yields(self):
        """Load current TIPS real yield curve from FRED API (parallelized).
        ...
        fetched: Dict[str, float] = {}
        with ThreadPoolExecutor(max_workers=len(self.TIPS_ENDPOINTS)) as executor:
            futures = {
                executor.submit(self._fred_fetch_series, series_id, api_key): tenor
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.debug(f"TIPS yield fetch failed for {tenor}: {e}")
            ...
            for holding in holdings:
                ...
                if self._is_bond(holding):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Bond {holding.get('cusip', 'unknown')} failed Holding validation: {e}"
                        )
    ...
    def load_bonds_from_holdings_json(self, json_path: Path) -> List[Dict]:
        """Load bonds from a CDM or legacy holdings.json file.

        Delegates to :class:`internal.holdings_loader.HoldingsLoader`. Bond
        positions are identified by ``asset_class == "bond"`` on the
        normalized Position. security_name-driven enrichment (coupon /
        maturity extracted from broker text) and Holding validation are
        preserved.
        """
        try:
            portfolio = HoldingsLoader().load(json_path)
            ...
            for pos in portfolio.positions:
                ...
                # Drop Nones to avoid confusing downstream fallback logic
                entry = {k: v for k, v in entry.items() if v is not None}
                bonds.append(entry)

            # Enrich bonds missing maturity/coupon from security_name
            for bond in bonds:
                ...
                if sec_name:
                    ...
                    if parsed.get("asset_type") and bond.get("asset_type") in (
                        "bond",
                        "fixed_income",
                        None,
                        "",
            ...
            # Validate via Holding interface (CDM-compatible)
            for bond in bonds:
                try:
                    _ = Holding.from_dict(bond)
                except Exception as e:
                    logger.warning(
                        f"Bond {bond.get('cusip', 'unknown')} failed Holding validation: {e}"
                    )

            logger.info(f"Loaded {len(bonds)} bonds from {Path(json_path).name}")
            return bonds
        except Exception as e:
    ...
    def _parse_security_name(name: str) -> Dict:
        ...
        if re.search(
            r"\bTREASURY\b|\bT[\s-]BILL\b|\bT[\s-]BOND\b|\bT[\s-]NOTE\b|\bTREASURIES\b", name_upper
        ):
    ...
    def _is_bond(self, holding: Dict) -> bool:
        ...
        maturity = (
            holding.get("maturity_date", "").strip()
            if isinstance(holding.get("maturity_date"), str)
            else holding.get("maturity_date")
        )
        ...
        if (
            (not symbol or symbol.lower() in ("none", "n/a", "nan", ""))
            and len(cusip) == 9
            and re.match(r"^[A-Za-z0-9]{9}$", cusip)
        ):
            return True
    ...
    def _calculate_ytm(
        ...
        # Zero-coupon fast path: brentq fails when coupon_rate=0.
        # All period calculations use max(1, round(...)) to handle very short maturities.
        if coupon_rate == 0:
            try:
    ...
    def _calculate_convexity(
        ...
        try:
            ...
            convexity = (pv_weighted_convexity / (pv_total * (1 + period_rate) ** 2)) / (
                frequency**2
            )
            return convexity
        except Exception as e:
        ...
        try:
            ...
            if maturity_date:
                try:
                    ...
                    logger.warning(
                        f"Could not parse maturity date '{maturity_date}': {e}. "
                        "Duration/YTM will be 0 and credit quality will show Unrated."
                    )
                    years_to_maturity = 0.0
            else:
                logger.warning(
                    f"Bond {symbol or cusip}: maturity date unknown — "
            ...
            market_value = safe_float(
                holding.get("market_value"), safe_float(holding.get("value"), 0.0)
            )
            price_percent = safe_float(
                holding.get("price_percent"), (market_value / par_value * 100) if par_value else 100
            )
            ...
            if ytm is None:
                logger.warning(
                    f"YTM calculation failed for {symbol}. Duration/convexity will not be calculated."
                )
                ...
                macaulay_dur, modified_dur = self._calculate_duration(
                    coupon_rate, ytm, years_to_maturity
                )
            ...
            credit_quality = self._estimate_credit_quality(
                ytm if ytm is not None else coupon_rate, coupon_rate
            )
    ...
    def analyze_portfolio(self, bonds: List[Dict]) -> Optional[PortfolioMetrics]:
        ...
        for holding in bonds:
            ...
            if metrics:
                ...
                # Credit quality
                quality_score = self.CREDIT_RATINGS.get(
                    metrics.credit_quality_estimate.split("/")[0], 10
                )
        ...
        if unknown_pct > 20:
            logger.warning(
                ...
                "YTM/duration averages exclude these bonds. Run '/ic-holdings' to refresh data."
            )

        # Calculate percentages
        for asset_type in asset_breakdown:
            asset_breakdown[asset_type]["pct"] = (
                asset_breakdown[asset_type]["value"] / total_value
            ) * 100

        for bucket in maturity_breakdown:
            maturity_breakdown[bucket]["pct"] = (
                maturity_breakdown[bucket]["value"] / total_value
            ) * 100
        ...
        avg_quality_score = (
            sum(credit_qualities) / len(credit_qualities) if credit_qualities else 10
        )
    ...
    def _generate_recommendations(
        ...
        if duration > 7:
            recs.append(
                "High interest rate sensitivity: Consider shortening duration if rates are expected to rise."
            )

        if bond_count < 5:
            recs.append(
                f"Limited diversification: Portfolio contains only {bond_count} bonds. Consider increasing."
            )
        ...
        for asset_type, data in asset_breakdown.items():
            if data["pct"] > 70:
                recs.append(
                    f"High concentration in {asset_type}: {data['pct']:.1f}% of portfolio. Consider diversifying."
                )

        if ytm < 2:
            recs.append(
                "Low portfolio yield: Consider duration extension or credit quality upgrade for higher income."
            )
        elif ytm > 6:
            recs.append(
                "High portfolio yield: Verify credit quality is appropriate for your risk tolerance."
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python3 bond_analyzer.py /path/to/portfolios/Holdings_extracted.csv /path/to/reports/bond_analysis.json"
        )
        sys.exit(1)
    ...
    output_file = (
        Path(sys.argv[2])
        if len(sys.argv) > 2
        else input_file.parent / f"{input_file.stem}_bond_analysis.json"
    )
    ...
    if not bonds:
        # Empty bond set is a valid state (e.g. equity-only portfolio), not an error.
        # Emit an empty analysis and exit 0 so downstream tooling gets a consistent envelope.
        empty_report = {
            "_note": "No bonds in portfolio; bond analysis skipped.",
            "bond_count": 0,
            "total_value": 0.0,
            "bonds": [],
        }
        try:
            ...
            output_file.write_text(json.dumps(empty_report, indent=2), encoding="utf-8")
        except Exception as exc:
            logger.warning(f"Could not write empty bond report: {exc}")
    ...
    _fred_note = (
        "FRED_KEY_MISSING — benchmark spreads unavailable; bond analysis uses coupon/price data only."
        if analyzer._fred_degraded
        else None
    )
    _compact_bond = {
    ...
    if is_interactive():
        ...
        print(format_header("Bond Portfolio Analysis"))
        print(f"  {Colors.CYAN}Bonds{Colors.RESET}: {_compact_bond['bond_count']}")
        print(
            f"  {Colors.CYAN}Total Value{Colors.RESET}: {format_currency(_compact_bond['total_value'])}"
        )
        print(
            f"  {Colors.CYAN}Weighted Avg YTM{Colors.RESET}: {format_percent(_compact_bond['weighted_avg_ytm_pct'])}"
        )
        print(
            f"  {Colors.CYAN}Weighted Avg Duration{Colors.RESET}: {_compact_bond['weighted_avg_duration_yrs']:.2f} years"
        )
        print(f"  {Colors.CYAN}Duration Risk{Colors.RESET}: {_compact_bond['duration_risk']}")
        print(
            f"  {Colors.CYAN}Credit Quality{Colors.RESET}: {_compact_bond['average_credit_quality']}"
        )
        if _compact_bond["annual_muni_tax_savings"] > 0:
            print(
                f"  {Colors.CYAN}Annual Muni Tax Savings{Colors.RESET}: {format_currency(_compact_bond['annual_muni_tax_savings'])}"
            )

        if _compact_bond.get("asset_type_breakdown"):
            print(format_header("Asset Type Breakdown", width=50))
            for atype, data in _compact_bond["asset_type_breakdown"].items():
                pct = data.get("pct", 0)
                print(
                    f"  {Colors.CYAN}{atype:15s}{Colors.RESET}: {data['count']:2d} bonds, {format_currency(data['value']):>12s} ({pct:5.1f}%)"
                )

        if _compact_bond.get("maturity_ladder"):
            print(format_header("Maturity Ladder", width=50))
            for bucket in ["0-2y", "2-5y", "5-10y", "10+y"]:
                data = _compact_bond["maturity_ladder"].get(bucket, {})
                if data.get("count", 0) > 0:
                    print(
                        f"  {Colors.CYAN}{bucket:5s}{Colors.RESET}: {data['count']:2d} bonds, {format_currency(data['value']):>12s} ({data.get('pct', 0):5.1f}%)"
                    )
    else:
