...
def _directive_hits(text: str) -> list[tuple[str, str]]:
    return [
        (m.group(0), desc)
        for pat, desc in _DIRECTIVE_PATS
...
class GuardedSession:
    ...
    def prime(self, silent: bool = False) -> bool:
        ...
        if not self.profile.prime_session:
            if not silent:
                print(
                    f"ℹ️  {self.profile.display} ({self.profile.level.value}) — no priming needed."
                )
            return False
    ...
    def query(
        ...
        if not ok:
            return (
                False,
                response,
                EnforcementResult(text=response, violations=["openclaw returned non-zero"]),
            )
...
def cmd_status():
    ...
    if _is_fa:
        ...
        _RST = "\033[0m"
        print(f"\n{_RED}{_BOLD}{'═' * 62}{_RST}")
        print(f"{_RED}{_BOLD}  InvestorClaw Guardrails — FA PROFESSIONAL (ADVISORY){_RST}")
        print(f"{_RED}{_BOLD}{'═' * 62}{_RST}")
        print(f"{_RED}  deployment_mode  : fa_professional — Dangerous Mode{_RST}")
        print(f"{_RED}  Active model     : {model or '(unknown)'}{_RST}")
        print(f"{_RED}  Profile          : {p.display}{_RST}")
        print(f"{_RED}  Enforcement      : {_badge(p.level)} + FA ADVISORY GUARDRAIL{_RST}")
        if p.issues:
            print(f"{_RED}  Known issues     :{_RST}")
            for issue in p.issues:
                print(f"{_RED}    • {issue}{_RST}")
        print()
        mech = [
        ...
        for name, active in mech:
            print(f"{_RED}  {name:<25} {'ON  ←' if active else 'off'}{_RST}")
        print(f"{_RED}{_BOLD}{'═' * 62}{_RST}\n")
    else:
        print(f"\n{'=' * 62}")
        print("  InvestorClaw Unified Guardrails — Session Status")
        print(f"{'=' * 62}")
...
def cmd_query(prompt: str, timeout: int, out_file: Optional[str], model_id: Optional[str]):
    ...
    print(
        f"[guardrails] model={session.profile.display} level={session.profile.level.value}",
        file=sys.stderr,
    )
    if session.profile.per_turn_inject:
        print("[guardrails] per-turn injection ACTIVE", file=sys.stderr)
...
def main():
    ...
    if "--query" in args:
        ...
        if idx + 1 >= len(args):
            print(
                'Usage: model_guardrails.py --query "prompt" [--timeout N] [--out file] [--model MODEL_ID]'
            )
            return 1
        prompt = args[idx + 1]
