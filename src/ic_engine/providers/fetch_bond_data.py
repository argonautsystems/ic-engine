...
class BondDataFetcher:
    ...
    def calculate_duration(
        self, coupon_rate: float, ytm: float, years_to_maturity: float, face_value: float = 100
    ) -> Tuple[float, float]:
        """
    ...
    def fetch_bond_etf_data(self, etf_symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                "asset_class": self.TREASURY_YIELDS.get(etf_symbol)
                or self.CORPORATE_BONDS.get(etf_symbol)
                or self.MUNICIPAL_BONDS.get(etf_symbol)
                or self.INFLATION_PROTECTED.get(etf_symbol)
                or "Bond ETF",
            }
    ...
    def analyze_portfolio_bonds(self, holdings_file: str, output_file: str = None) -> Dict:
        ...
        try:
            ...
            for bond in bonds:
                ...
                shares = float(
                    bond.get("quantity", bond.get("shares", 1))
                )  # For bonds, usually quantity
                ...
                ytm = self.calculate_ytm(
                    current_price, coupon_rate, face_value=100, years_to_maturity=years_to_maturity
                )

                # Calculate Duration
                macaulay_dur, modified_dur = self.calculate_duration(
            ...
            if total_value > 0:
                ...
                avg_credit = (
                    sum(credit_qualities) / len(credit_qualities) if credit_qualities else 0
                )
            ...
            report = {
                ...
                "portfolio_summary": {
                    ...
                    "average_macaulay_duration": float(
                        duration_weighted / total_value if total_value > 0 else 0
                    ),
            ...
            if output_file:
                ...
                try:
                    ...
                    _dep_mode = None
                wrapped_report = DisclaimerWrapper.wrap_output(
                    report, "Bond Portfolio Analysis", deployment_mode=_dep_mode
                )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python3 fetch_bond_data.py ~/portfolio_reports/holdings.json ~/portfolio_reports/bond_analysis.json"
        )
        sys.exit(1)
